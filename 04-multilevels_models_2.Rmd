...
table1::table1(~ Sex + Age_class + Ethnicity +
                 Qualification + Urban | Loneliness,
       data=mydata,
...
# multilevel model
m_null <- glmmTMB(Loneliness ~ 1 + (1 | strata_2),
                  data = mydata,
                  family = binomial)
...
predictions <- predict_response(
    ...
    print(predictions %>% 
            ...
            labs(y = "Sratas",
                 ...
                 color = "Sex category",
                 linetype = "Ethnicity",
                 title = "Predicted values for Loneliness") +
...
m_gender <- glmmTMB(Loneliness ~ Sex + (1 | strata_2),
                    data = mydata, family=binomial)
m_ethnicity <- glmmTMB(Loneliness ~ Ethnicity + (1 | strata_2),
                       data = mydata, family=binomial)
m_age <- glmmTMB(Loneliness ~ Age_class + (1 | strata_2),
                 data = mydata, family=binomial)
m_urban <- glmmTMB(Loneliness ~ Urban + (1 | strata_2),
                       data = mydata, family=binomial)
m_qualif <- glmmTMB(Loneliness ~ Qualification + (1 | strata_2),
                 data = mydata, family=binomial)
m_full <- glmmTMB(Loneliness ~ Age_class + Ethnicity + Sex +
